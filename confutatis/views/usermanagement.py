...
from confutatis import view, Log, DataSet, User, Auth, Error, localization
...
class UserSettingsModify(UserSettingsDisplay):
    ...
    def render(self):
        ...
        if login != self.user.login and not self.user.has(User.PERM_USER_MANAGEMENT):
            raise Error.ConfutatisUserError("Permission Denied", "Cannot modify other users settings", log=Log.WARNING)
        ...
        if not lang in localization.getLanguagesIdentifiers():
            raise Error.ConfutatisUserError("Invalid Language", "Specified language does not exist", log=Log.WARNING)
        ...
        if self.parameters.has_key("password_new") and self.parameters.has_key("password_new_confirmation"):
            if login == self.user.login:
                try:
                    ...
                    raise Error.ConfutatisUserError("Password Error", "Invalid Password specified")

            if self.parameters["password_new"] != self.parameters["password_new_confirmation"]:
                raise Error.ConfutatisUserError("Password Error", "Password mismatch")
