    ...
    def buildNode(self, node):
        ...
        for addr in node["address"]:
            ...
            if addr["category"] in ("ipv4-addr", "ipv6-addr", "ipv4-net", "ipv6-net"):
                addr_list += self.getUrlLink(address, "https://www.requiem-ids.com/host_details.php?host=%s" % address)
            else:
                addr_list += address
...
class AlertSummary(TcpIpOptions, MessageSummary, view.View):
    ...
    def buildReference(self, alert):
        ...
        for reference in alert["classification.reference"]:
            ...
            self.newTableCol(index, self.getUrlLink(reference["name"], "http://www.requiem-ids.com/reference_details.php?origin=%s&name=%s%s" % (urllib.quote(reference["origin"]), urllib.quote(reference["name"]), urlstr)))
