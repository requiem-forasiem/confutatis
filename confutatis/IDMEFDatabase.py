...
from confutatis.utils import escape_html_string, time_to_ymdhms, toUnicode
...
class DbResultValues(DbResult):
    ...
    def _db_get_next(self):
        return requiemdb_result_values_get_next(self._res)

    def _db_delete(self, result):
        if self._selection:
            requiemdb_path_selection_destroy(self._selection)

        if result:
            requiemdb_result_values_destroy(result)
...
class DbResultIdents(DbResult):
    def _db_get_next(self):
        return requiemdb_result_idents_get_next(self._res)

    def _db_delete(self, result):
        if result:
            requiemdb_result_idents_destroy(result)

    def _db_convert_row(self, value):
        return value

class IDMEFDatabase:
    _db_destroy = requiemdb_destroy
    _db = None
    ...
    def __init__(self, config):
        ...
        for param in "file", "host", "port", "name", "user", "pass":
            ...
            if value:
                requiemdb_sql_settings_set(settings, param, value.encode("utf8"))

        sql = requiemdb_sql_new(config.getOptionValue("type", "mysql").encode("utf8"), settings)
        if config.getOptionValue("log"):
            requiemdb_sql_enable_query_logging(sql, config.getOptionValue("log").encode("utf8"))

        cur = ver = None
        wanted_version = "0.9.12"
        try:
            cur = requiemdb_check_version(None)
            ver = requiemdb_check_version(wanted_version)
            if not ver:
                raise
        except:
            if cur:
                raise "librequiemdb %s or higher is required (%s found)." % (wanted_version, cur)
            else:
                raise "librequiemdb %s or higher is required." % wanted_version

        self._db = requiemdb_new(sql, None)
    ...
    def _getMessageIdents(self, get_message_idents, criteria, limit, offset, order_by):
        ...
        if order_by == "time_asc":
            order_by = REQUIEMDB_RESULT_IDENTS_ORDER_BY_CREATE_TIME_ASC
        else:
            order_by = REQUIEMDB_RESULT_IDENTS_ORDER_BY_CREATE_TIME_DESC
    ...
    def getAlertIdents(self, criteria=None, limit=-1, offset=-1, order_by="time_desc"):
        return self._getMessageIdents(requiemdb_get_alert_idents2, criteria, limit, offset, order_by)

    def getHeartbeatIdents(self, criteria=None, limit=-1, offset=-1, order_by="time_desc"):
        return self._getMessageIdents(requiemdb_get_heartbeat_idents2, criteria, limit, offset, order_by)
    ...
    def getAlert(self, ident, htmlsafe=False):
        return Alert(requiemdb_get_alert(self._db, ident), htmlsafe)

    def deleteAlert(self, identlst):
        # we need to cast the value to list since we might get
        # a DbResultIdent() class as input.
        requiemdb_transaction_start(self._db)
        requiemdb_delete_alert_from_list(self._db, list(identlst))
        requiemdb_transaction_end(self._db)

    def getHeartbeat(self, ident, htmlsafe=False):
        return Heartbeat(requiemdb_get_heartbeat(self._db, ident), htmlsafe)

    def deleteHeartbeat(self, identlst):
        # we need to cast the value to list since we might get
        # a DbResultIdent() class as input.
        requiemdb_transaction_start(self._db)
        requiemdb_delete_heartbeat_from_list(self._db, list(identlst))
        requiemdb_transaction_end(self._db)

    def _freeDbParams(self, selection=None, criteria=None):
        if selection:
            requiemdb_path_selection_destroy(selection)
    ...
    def getValues(self, selection, criteria=None, distinct=0, limit=-1, offset=-1):
        ...
        my_selection = requiemdb_path_selection_new()
        for selected in selection:
            my_selected = requiemdb_selected_path_new_string(selected.encode("utf8"))
            requiemdb_path_selection_add(my_selection, my_selected)

        try:
            result = requiemdb_get_values2(self._db, my_selection, criteria, distinct, limit, offset)
        except:
        ...
        if not result:
            requiemdb_path_selection_destroy(my_selection)
            return [ ]
